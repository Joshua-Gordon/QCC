...
import javafx.appUI.GateIcon;
import javafx.appUI.LatexNode;
import javafx.appUI.MainScene;
import javafx.appUI.appViews.AppView.ViewListener;
import javafx.appUI.appViews.gateChooser.AbstractGateChooser;
...
import javafx.framework.exportGates.Control;
import javafx.framework.exportGates.RawExportableGateData;
import javafx.framework.gateModels.CircuitBoardModel;
import javafx.framework.gateModels.GateModel;
import javafx.framework.gateModels.PresetGateType.PresetGateModel;
import javafx.framework.solderedGates.SolderedGate;
		...
		public void toolAction() {
			...
			if(gm != null) { 
				if(gm.getNumberOfRegisters() == 1) {
					...
					try {
						circuitBoard.placeGate(gm.getFormalName(), column, new Integer[]{row}, params);
...
//	Render Methods:
	...
	public Node makeRegularGateBody(RawExportableGateData data, String symbol) {
		...
		GridPane.setConstraints(bp, data.getColumn() + 1, data.getGateRowBodyStart(), 1, data.getGateRowBodyStart() - data.getGateRowBodyEnd() + 1);
	...
	private void setOnSolderHover(Node n, int row, int column) {
		...
		n.setOnMouseClicked(solderRegion);
	}
